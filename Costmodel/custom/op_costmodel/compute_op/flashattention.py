...
@op_manager.register("FlashAttention", "npu_fusion_attention", "FusionAttention")
class FlashAttentionPrediction(BaseOp):
    def __init__(self, op, hardware, layout="bshd"):
        """
        ...
        if self.op.name in ["npu_fusion_attention", "FusionAttention"]:  # training
            self.op_type = "mix"
            self.mask_ratio = 0.5

            q_shape, k_shape, v_shape, *_ = self.inputs_shape
            self.q_dtype, self.k_dtype, self.v_dtype, *_ = self.inputs_dtype

            if len(layout) == 4:
                if len(q_shape) == 3:
                    self.batch = 1
                    self.num_heads = q_shape[1]
                    self.q_seq_len = q_shape[0]
                    self.k_seq_len = k_shape[0]
                    self.head_dim = q_shape[-1]
                    self.v_dim = v_shape[-1]
                else:
                    assert f"FA costmodel for current shape is not implemented. Op: {self.op}"
            elif len(layout) == 3:
                self.batch = q_shape[0]
                self.num_heads = 1  # does not effect flops
                self.q_seq_len = q_shape[1]
                self.k_seq_len = k_shape[1]
                self.head_dim = q_shape[-1]
                self.v_dim = v_shape[-1]
            else:
                assert f"FA costmodel for layout {layout} is not implemented. Op: {self.op}"
        else:
            ...
            self.q_seq_len = self.q_shape[dim_map["s"]]
            self.k_seq_len = self.k_shape[dim_map["s"]]
    ...
    def _create_flash_attention_predictors(self):
        ...
        batch, num_heads, q_seq_len, k_seq_len, head_dim = (
            self.batch,
            self.num_heads,
            self.q_seq_len,
            self.k_seq_len,
        ...
        q_one_head_shape = [batch, q_seq_len, head_dim]
        transpose_k_one_head_shape = [batch, head_dim, k_seq_len]

        q_mul_k_one_head_op = create_op_info(
            input_list=[q_one_head_shape, transpose_k_one_head_shape],
            input_dtype_list=[q_dtype, k_dtype],
            output_list=[[batch, q_seq_len, k_seq_len]],
        ...
        attn_scores_shape = [batch, q_seq_len, k_seq_len]
...
@op_manager.register("FlashAttentionActivationBackward", "npu_fusion_attention_grad", "FusionAttentionGrad")
class FlashAttentionBackwardPrediction(BaseOp):
    def __init__(self, op, hardware, layout="bshd"):
        ...
        """
        super().__init__(op, hardware)
        if self.op.name in ["npu_fusion_attention_grad", "FusionAttentionGrad"]:  # training
            self.op_type = "mix"
            self.mask_ratio = 0.5
            ...
            if len(layout) == 4:
                if len(q_shape) == 3:
                    ...
                    assert f"FA grad costmodel for current shape is not implemented. Op: {self.op}"
            elif len(layout) == 3:
                self.batch = q_shape[0]
                self.num_heads = 1  # does not effect flops
                self.q_seq_len = q_shape[1]
                ...
                assert f"FA grad costmodel for layout {layout} is not implemented. Op: {self.op}"
    ...
    def _create_flash_attention_backward_predictors(self):
        ...
        v_dim = self.v_dim
        # Attention score  recomputed
        q_one_head_shape = [batch, self.q_seq_len, head_dim]
        transpose_k_one_head_shape = [batch, head_dim, self.k_seq_len]

        q_mul_k_one_head_op = create_op_info(
            input_list=[q_one_head_shape, transpose_k_one_head_shape],
            input_dtype_list=[q_dtype, k_dtype],
            output_list=[[batch, self.q_seq_len, self.k_seq_len]],
            output_dtype_list=[q_dtype],
        )
        qk_predictor = MatmulPrediction(q_mul_k_one_head_op, self.hardware)
        ...
        return qk_predictor, score_grad_predictor, v_grad_predictor, softmax_predictor, q_grad_predictor, k_grad_predictor
    ...
    def cube_flops_dict(self):
        ...
        qk_predictor, score_grad_predictor, v_grad_predictor, _, q_grad_predictor, k_grad_predictor = (
            self._create_flash_attention_backward_predictors()
        )
        qk_recompute_flops = {k: v * num_heads * mask_ratio for k, v in qk_predictor.cube_flops_dict.items()}
        ...
        total_flops_dict = merge_flops_dicts([qk_recompute_flops, score_grad_flops, v_grad_flops, q_grad_flops, k_grad_flops])
    ...
    def vec_flops_dict(self):
        ...
        _, _, _, softmax_grad_predictor, _, _ = self._create_flash_attention_backward_predictors()
