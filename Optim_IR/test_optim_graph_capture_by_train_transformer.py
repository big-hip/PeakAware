...
# 捕获图
""" 用户参考 start"""
graph_capture = Dist_IR.GraphCapture(model,src_data, tgt_data[:, :-1])
...
for epoch in range(num_epochs):
    outputs = model(src_data, tgt_data[:, :-1])
    loss = criterion(outputs.contiguous().view(-1, tgt_vocab_size), tgt_data[:, 1:].contiguous().view(-1))
