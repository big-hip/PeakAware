...
def main() -> None:
    ...
    import torch_npu
    torch_npu.__version__ = "2.9.0"  # te_fusion workaround
    ...
    inputs = build_input(args.model, args.batch, torch.device("npu"))
    ...
    for _ in range(args.steps):
        ...
        one_step()
        torch.npu.synchronize()
        peaks.append(int(torch.npu.max_memory_allocated()))
    peak = max(peaks)

    torch.npu.reset_peak_memory_stats()
    logits_squared_mean_loss(model(*inputs)).backward()
    torch.npu.synchronize()
    fw_bw_peak = int(torch.npu.max_memory_allocated())
